...
def _normalize_migration_url(url: str) -> str:
    """Align the URL with the app's async driver so migrations use asyncpg.

    Mirrors ``app.database.session._normalize_database_url``: the application
    runs on SQLAlchemy async (asyncpg / aiosqlite), and psycopg2 is not a
    project dependency, so plain ``postgresql://`` / ``postgres://`` URLs are
    mapped to the asyncpg driver the runtime actually uses.
    """
    clean = (url or "").strip()
...
DATABASE_URL = _normalize_migration_url(DATABASE_URL)
