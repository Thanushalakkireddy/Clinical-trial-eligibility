...
# Providers documented locally: backend/.env uses ``xkiro`` as the xAI
# provider name, so normalize known aliases before any provider checks.
_PROVIDER_ALIASES = {"xai": "xai", "xkiro": "xai", "gemini": "gemini"}


def _normalized_provider(cfg: Settings) -> str:
    """Lower-cased provider name with recognized aliases normalized (xkiro -> xai)."""
    raw = (cfg.llm_provider or "xai").strip().lower()
...
def llm_is_configured(cfg: Settings) -> bool:
    ...
    provider = _normalized_provider(cfg)
    if provider == "xai":
...
def validate_runtime_config(cfg: Optional[Settings] = None) -> List[str]:
    ...
    provider = _normalized_provider(c)
    if provider == "xai":
