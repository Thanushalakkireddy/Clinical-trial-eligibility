...
# config.py lives in backend/app/, so the backend root is two parents up.
_BACKEND_DIR = Path(__file__).resolve().parent.parent


def dotenv_path() -> Path:
    """Absolute path to ``backend/.env``, independent of the process CWD.

    Anchoring to this module's location (rather than the working directory)
    guarantees local development always finds ``backend/.env`` whether uvicorn
...
class Settings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        # Absolute path derived from the backend package location, so discovery
        # never depends on the current working directory. pydantic-settings
        # gives real environment variables precedence over values in this file,
        # which preserves Render's behavior where only platform variables exist.
